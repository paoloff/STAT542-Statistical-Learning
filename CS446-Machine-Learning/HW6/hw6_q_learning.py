...
class Agent:
    ...
    def _act_train(self, state, epsilon):
        ...
        if random.uniform(0,1) > epsilon:
            #action = np.argmax(self.q_table[state[0], np.where(state[1]["action_mask"] == 1)])
    ...
    def update(self, state, action, reward, next_state, alpha, gamma):
        ...
        """
        y = reward + gamma*np.max(self.q_table[next_state,:])
        self.q_table[state, action] = (1-alpha)*self.q_table[state, action] + alpha*y
...
def q_learning(alpha=0.1, gamma=0.9, epsilon=0.1):
    ...
    plt.savefig("./train_moving_avg2.png")
